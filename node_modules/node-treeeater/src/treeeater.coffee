...
class Git
    constructor: (@opts) ->
        for cmd in (require './git-commands').commands
    ...
    commit_tree_hierachy: (tree_hierachy, opts..., cb) =>
        ...
        todo = 0
        blobs = {}
        for filename, blob of tree_hierachy.all
        ...
        commits.on 'item', (commit) =>
            if todo
                for filename of commit.changes
                    if filename of blobs
                        blobs[filename].commit = commit
                        ee.emit 'item', blobs[filename]
    ...
    cat: (treeish, opts..., cb) =>
        ...
        if typeof treeish == 'string'
            ...
            revision = k
        [ opts, cb ] = @opts_cb opts, cb
        @cat_file '-p', opts, "#{revision}:#{filename}", chunked: true, cb
        ...
        if chunked
            if parser
                ...
                buffer.split '\n', (l,t) ->
                    ...
                    (s.emit 'data', data) if item or first_time
                    first_time = no if first_time
                buffer.on 'close', ->
                    item = parser.end()
                    (s.emit 'data', JSON.stringify(item) + "]") if item
                    s.emit 'close'
                #if cb
                #    items = []
...
class ItemsParser
    ...
    line: (line) =>
        ...
        unless matched
            console.log "ItemsParser.line - unknown line:", line
        return_item
...
class CommitsParser extends ItemsParser
    ...
    regexes = [
        ...
        [/^:(\S+) (\S+) ([0-9a-z]+) ([0-9a-z]+) (.+)\t(.+)/, (match) ->
            [ _, modea, modeb, shaa, shab, status, filename ] = match
            (@item.changes ?= {})[filename] = { modea, modeb, shaa, shab, status }]
        [/^([0-9-]+)\s+([0-9-]+)\s+(.+)/, (match) ->
            [ _, plus, minus, filename ] = match
            (@item.numstats ?= {})[filename] = { plus, minus }]
        [/^$/, ->]
    ]
...
class TreesParser extends ItemsParser
    ...
    regexes = [
        [/^(\S+) (\S+) (\S+)\s+(\S+)\s+(.+)/, (match) ->
            [ _, mode, type, sha, size, filename ] = match
            @item = { mode, type, sha, size, filename }]]
