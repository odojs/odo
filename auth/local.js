...
define(['passport', 'passport-local', 'node-uuid', 'odo/redis', 'bcryptjs', 'odo/config', 'odo/hub', 'odo/express', 'odo/inject'], function(passport, passportlocal, uuid, redis, bcrypt, config, hub, express, inject) {
  var LocalAuthentication;
  return LocalAuthentication = (function() {
    function LocalAuthentication() {
      this.remove = bind(this.remove, this);
      this.assignpassword = bind(this.assignpassword, this);
      this.assignusername = bind(this.assignusername, this);
      this.signup = bind(this.signup, this);
      this.reset = bind(this.reset, this);
      this.generateresettoken = bind(this.generateresettoken, this);
      this.getresettoken = bind(this.getresettoken, this);
      this.usernameavailability = bind(this.usernameavailability, this);
      this.emailavailability = bind(this.emailavailability, this);
      this.test = bind(this.test, this);
      this.signin = bind(this.signin, this);
      this.auth = bind(this.auth, this);
      this.projection = bind(this.projection, this);
      this.updateemail = bind(this.updateemail, this);
    ...
    LocalAuthentication.prototype.projection = function() {
      ...
      hub.every('remove local signin from user {id}', (function(_this) {
        return function(m, cb) {
          return _this.db().hdel(config.odo.domain + ":localusers", m.username, function() {
            return cb();
          });
        };
      })(this));
    ...
    LocalAuthentication.prototype.auth = function(req, res, next) {
      return passport.authenticate('local', function(err, user, info) {
        ...
        if (!user) {
          if (((ref = config.odo.auth) != null ? (ref1 = ref.local) != null ? ref1.failureRedirect : void 0 : void 0) != null) {
            return res.redirect(config.odo.auth.local.failureRedirect);
          }
          return res.redirect('/#auth/local/failure');
        }
        return req.logIn(user, function(err) {
          var ref2, ref3, ref4, returnTo;
          if (err != null) {
            return next(err);
          }
          if (((ref2 = req.session) != null ? ref2.returnTo : void 0) != null) {
            returnTo = req.session.returnTo;
            delete req.session.returnTo;
            return res.redirect(returnTo);
          }
          if (((ref3 = config.odo.auth) != null ? (ref4 = ref3.local) != null ? ref4.successRedirect : void 0 : void 0) != null) {
    ...
    LocalAuthentication.prototype.signup = function(req, res) {
      var displayName, email, profile, userid, username;
        ...
        return inject.one('odo user by id')(userid, (function(_this) {
          return function(err, user) {
            ...
            return req.login(user, function(err) {
              ...
              if (((ref = req.session) != null ? ref.returnTo : void 0) != null) {
                returnTo = req.session.returnTo;
                delete req.session.returnTo;
                return res.redirect(returnTo);
              }
    ...
    LocalAuthentication.prototype.assignusername = function(req, res) {
      ...
      this.db().hset(config.odo.domain + ":localusers", req.body.username, req.body.id, (function(_this) {
        return function(err, reply) {
          if (err != null) {
            return res.send(400, "set error on localusers ");
          }
          return _this.db().hdel(config.odo.domain + ":localusers", req.user.username, function(err, reply) {
            if (err != null) {
              return res.send(400, "delete error on localusers");
            }
          });
        };
      })(this));
      ...
      return hub.emit('assign username {username} to user {id}', p, function() {
        return res.send('Ok');
      });
    };
    ...
    LocalAuthentication.prototype.assignpassword = function(req, res) {
      ...
      return hub.emit('set password of user {id}', p, function() {
        return res.send('Ok');
      });
    };
      ...
      return this.db().hget("" + config.odo.domain + ":localusers", username, (function(_this) {
        return function(err, userdata) {
          ...
          if (userdata != null) {
            ...
            return _this.db().hget("" + config.odo.domain + ":localemails", username, function(err, emaildata) {
              if (err != null) {
                return callback(err);
              }
